...
class TurnstileEvent(_Base):
    ...
    def _no_future(cls, v: datetime) -> datetime:
        # Reject events claiming to be from the future (clock-skew / replay attack)
...
class DensityReading(_Base):
    ...
    density: Annotated[int, Field(ge=0, le=10000)]            # persons / 100m²
...
class WeatherSignal(_Base):
    """Open-Meteo poll output. Triggers CrowdFlowAgent re-plan on rain."""
    event_id: str = Field(default_factory=lambda: str(uuid.uuid4()))
...
class IngestAck(_Base):
    accepted: bool
    event_id: str
    stream_id: str | None = None   # Redis XADD return ID, null on duplicate/error
